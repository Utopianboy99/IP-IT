...
export const publicApiRequest = async (url, options = {}) => {
  ...
  try {
    ...
    // If the server responds 401 for a public endpoint, but we have a signed-in Firebase user,
    // retry the request using the authenticated apiRequest (adds Authorization header).
    if (response.status === 401) {
      ...
      if (user) {
        ...
        // use apiRequest to perform authenticated retry
